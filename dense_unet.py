...
class DenseUNet(nn.Module):
    def __init__(self, n_classes=1, growth_rate=32, block_config=(6, 12, 24, 16), num_init_features=64, bn_size=4, drop_rate=0, downsample=False, pretrained_encoder_uri=None, progress=None):
        ...
        self.output_layer = nn.Conv2d(num_init_features, n_classes, kernel_size=1, stride=1, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.decoder(x)
        #y = self.classifier(x)
        return self.output_layer(x) #self.softmax(y)